...
export const Radios: FC<RadiosProps> = ({ items, activeId, setActiveId }) => {
  return (
    <div>
      {items.map((radioElem) => {
        const isActive = radioElem.id === activeId;
        return (
          <label className={style.label} key={radioElem.id}>
            <input
              checked={radioElem.id === activeId}
              onChange={() => setActiveId(radioElem.id)}
              type="radio"
            ...
            <div className={style.svgs}>
              <svg
                className={classNames([
                  style.radiosSvg,
                  isActive && style.radiosSvgActive,
                ])}
                width="20"
                height="20"
                viewBox="0 0 20 20"
